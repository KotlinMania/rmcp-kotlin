...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Rmcp.swiftmodule/`
...
//      `import Rmcp` above would fail at compile time.
//
//   2. The static archive `libRmcp.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Rmcp" }` and
//      `flattenPackage = "io.github.kotlinmania.rmcp"` configuration in
...
final class RmcpExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Rmcp swift module imported cleanly")
    }
}
