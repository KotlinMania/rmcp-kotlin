# Claude Code Project Instructions — rmcp-kotlin

## Project Overview

This is **rmcp-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rmcp`](https://crates.io/crates/rmcp).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rmcp/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rmcp>
